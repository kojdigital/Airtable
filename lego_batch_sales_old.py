...
def airtable_patch():
    ...
    sheet = client.open("LEGO STOCK").get_worksheet(1)
        ...
        # for record in final_records_list:
            ...
            # print(str(row.get('SKU')))
                ...
                if record.get('fields').get('PARENT ITEM') == float(row.get('RMS PARENT ITEM')):
                    ...
                    fields_body['KSA VIRTUAL WAREHOUSE - NET'] = float(str(row.get('KSA VIRTUAL WAREHOUSE NET')).replace(",","").replace("~","0000"))
                    fields_body['KSA VIRTUAL WAREHOUSE - SOH'] = float(str(row.get('KSA VIRTUAL WAREHOUSE')).replace(",","").replace("~","0000"))
                    fields_body['KSA ECOM VIRTUAL WAREHOUSE - NET'] = float(str(row.get('KSA ECOM VIRTUAL WAREHOUSE NET')).replace(",","").replace("~","0000"))
                    fields_body['KSA ECOM VIRTUAL WAREHOUSE - SOH'] = float(str(row.get('KSA ECOM VIRTUAL WAREHOUSE')).replace(",","").replace("~","0000"))
                    fields_body['LGO - DHAHRAN - MALL OF DHAHRAN - NET'] = float(str(row.get('LGO - DHAHRAN - MALL OF DHAHRAN NET')).replace("SAR ","").replace(",","").replace("~","0000"))
                    fields_body['LGO - DHAHRAN - MALL OF DHAHRAN - SOH'] = float(str(row.get('LGO - DHAHRAN - MALL OF DHAHRAN')).replace(",","").replace("~","0000"))
                    fields_body['LGO - RIYADH - NAKHEEL MALL 2 - NET'] = float(str(row.get('LGO - RIYADH - NAKHEEL MALL 2 NET')).replace(",","").replace("~","0000"))
                    fields_body['LGO - RIYADH - NAKHEEL MALL 2 - SOH'] = float(str(row.get('LGO - RIYADH - NAKHEEL MALL 2')).replace(",","").replace("~","0000"))
                    fields_body['LGO - RIYADH - GRANADA CENTER - NET'] = float(str(row.get('LGO - RIYADH - GRANADA CENTER NET')).replace(",","").replace("~","0000"))
                    fields_body['LGO - RIYADH - GRANADA CENTER - SOH'] = float(str(row.get('LGO - RIYADH - GRANADA CENTER')).replace(",","").replace("~","0000"))
                    fields_body['LGO - JEDDAH - RED SEA MALL - NET'] = float(str(row.get('LGO - JEDDAH - RED SEA MALL NET')).replace(",","").replace("~","0000"))
                    fields_body['LGO - JEDDAH - RED SEA MALL - SOH'] = float(str(row.get('LGO - JEDDAH - RED SEA MALL')).replace(",","").replace("~","0000"))
                    fields_body['LGO - JEDDAH - SOUQ.COM - NET'] = float(str(row.get('LGO - JEDDAH - SOUQ.COM NET')).replace(",","").replace("~","0000"))
                    fields_body['LGO - JEDDAH - SOUQ.COM - SOH'] = float(str(row.get('LGO - JEDDAH - SOUQ.COM')).replace(",","").replace("~","0000"))
                    fields_body['LGO - RIYADH - PARK MALL - NET'] = float(str(row.get('LGO - RIYADH - PARK MALL NET')).replace(",","").replace("~","0000"))
                    fields_body['LGO - RIYADH - PARK MALL - SOH'] = float(str(row.get('LGO - RIYADH - PARK MALL')).replace(",","").replace("~","0000"))

                    #New entry BY SK/Reuben on 27-SEP-2021

                    fields_body['LGO - MAKKAH - MAKKAH MALL - NET'] = float(str(row.get('LGO - MAKKAH - MAKKAH MALL NET')).replace(",","").replace("~","0000"))
                    fields_body['LGO - MAKKAH - MAKKAH MALL - SOH'] = float(str(row.get('LGO - MAKKAH - MAKKAH MALL')).replace(",","").replace("~","0000"))
